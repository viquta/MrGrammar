...
| `MAX_CORRECTION_ATTEMPTS` | `3` | Attempt number at which the solution is revealed and the error is auto-resolved |

### Flow Description

The workflow supports four distinct scenarios, plus an alternative path for explicit solution requests.

#### Scenario A: Attempt 1 — Incorrect, No Hint

1. The student clicks a highlighted error in the submission text and types a correction (e.g., `"die"` for an article error where the correct answer is `"das"`).
...
5. Since `attempt_number = 1` which equals `HINT_THRESHOLD = 1`, depending on configuration this may or may not show a hint. With the default configuration where hint is shown at attempt ≥ `HINT_THRESHOLD`, i.e. after the first failed attempt — see Scenario B.
6. The service creates a `CorrectionAttempt` record (`attempt_number=1`, `is_correct=false`).
7. The response returns `{attempt_number: 1, is_correct: false, is_resolved: false}`.

#### Scenario B: Attempt 2 — Incorrect, Hint Shown

1. The student tries again with `"dem"`.
2. The service computes Levenshtein ratio(`"dem"`, `"das"`) = 0.33 → **incorrect**.
3. Since `attempt_number = 2 ≥ HINT_THRESHOLD = 1`, the service includes the `hint_text` from the `DetectedError` record in the response.
4. The service creates a `CorrectionAttempt` record (`attempt_number=2`, `is_correct=false`, `hint_shown=true`).
5. The response includes the hint: `{attempt_number: 2, is_correct: false, is_resolved: false, hint: "The noun requires a definite neuter article."}`.
6. The frontend displays the hint to the student.
...
4. The response returns `{attempt_number: 3, is_correct: true, is_resolved: true}`.
5. The frontend removes the error highlight and shows a success indicator.

#### Scenario D: Max Attempts Reached — Solution Revealed

If the student exhausts all `MAX_CORRECTION_ATTEMPTS` (default: 3) without a correct answer:

1. On the final incorrect attempt, the service includes **both** the hint and the correct solution in the response.
2. The error is auto-resolved (`is_resolved = true`).
3. The response returns: `{attempt_number: 3, is_correct: false, is_resolved: true, hint: "...", solution: "das"}`.
4. The frontend displays the solution and marks the error as resolved.

#### Alternative: Explicit Solution Request

At any point before resolution, the student may click "Show Solution":

1. The frontend sends `POST /api/feedback/errors/{id}/solution/`.
2. The `RequestSolutionView` retrieves the error's `hint_text` and `correct_solution`, marks the error as resolved, and returns both immediately.
3. The response returns: `{hint: "...", solution: "das"}`.
